How to run your own Denarius Electrum server
===================================
...
you also need to run denariusd and keep a copy of the full blockchain,
which is roughly 1.1 GB (September 2017). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit denariusd 
...
only leaving spare cycles for denariusd. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running denariusd and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `denariusd` and Electrum.
...
### Step 2. Download and compile denariusd

Please see https://github.com/carsenk/denarius for detailed instructions.


### Step 3. Configure and start denariusd

In order to allow Electrum to "talk" to `denariusd`, we need to set up an RPC
username and password for `denariusd`. We will then start `denariusd` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.denarius
    $ $EDITOR ~/.denarius/denarius.conf

Write this in `denarius.conf`:
...
If you have an existing installation of denariusd and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ denariusd -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `denariusd`:

    $ denariusd

Allow some time to pass for `denariusd` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ denariusd getblockchaininfo

Before starting the Electrum server your denariusd should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start denariusd at boot
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/carsenk/denariium-server.git
    $ cd denariium-server
...
and further instructions from the Electrum full archival server foundry is currently not available.
...
1. To increase privacy of transactions going through your server
   you may want to close denariusd for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting denariusd (together with electrum-server) on a weekly basis to clear out unconfirmed
...
Gitter, Slack, or Telegram and we'll try to congratulate you
on supporting the community by running a Denarius Electrum node.

If you're operating a public Denarius Electrum server please make a pull request
and add your server info to the electrum client README (at the bottom)
https://github.com/carsenk/denariium-server
